...
class CollageEnv(gym.Env):
    def __init__(self, target_image_paths, num_shapes, canvas_size, discriminator=None, gan_weight=0.1, gan_buffer=None, reward_type="mse"):
        ...
        self.n_actions = 20  
        #self.action_space = gym.spaces.MultiDiscrete([len(self.renderer.shape_textures), self.n_actions, self.n_actions, self.n_actions, self.n_actions, self.n_actions, self.n_actions, self.n_actions, self.n_actions])
        # 预设动作参数表
    ...
    def step(self, action):
        ...
        shape_type = action[0]  # 不归一化，直接作为类型索引
        param = action[1:] / (self.n_actions - 1)  # 其余参数归一化到[0,1]
        action_param = np.concatenate([[shape_type], param])
