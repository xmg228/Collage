...
class CollageEnv(gym.Env):
    def __init__(self, target_image_paths, num_shapes, canvas_size, discriminator=None, gan_weight=0.1, gan_buffer=None, reward_type="ssim"):
        ...
        self.num_shape_types = len(self.renderer.shape_textures)  # 假设有10种素材类型
        self.action_space = gym.spaces.Box(low=-1, high=1, shape=(self.num_shape_types + 8,), dtype=np.float32)
        # 观察空间: 当前画布 + 目标图像 (6通道)，float32，[0,1]
        obs_shape = (9, canvas_size, canvas_size)
        ...
        get_obs_shape( self.observation_space )
    ...
    def reset(self, *, seed=None, options=None):
        ...
        goal = self.target  # 目标图像
        achieved_goal = self.canvas  # 当前画布
        return {
            "observation": obs,
            "desired_goal": goal,
    ...
    def step(self, action):
        action = (action + 1) / 2  
        logits = action[:self.num_shape_types]
        shape_type = int(np.argmax(logits))  # 或 softmax采样
        ...
        obs = self._get_obs()  # 新观测
        achieved_goal = self.canvas
        reward = self.compute_reward(achieved_goal, self.target, info={})
        return {
            "observation": obs,
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        achieved_goal = achieved_goal.unsqueeze(0).to(self.device)
        desired_goal = desired_goal.unsqueeze(0).to(self.device)
        reward = self.loss_fn(achieved_goal, desired_goal).item() # 相似度计算
        return reward